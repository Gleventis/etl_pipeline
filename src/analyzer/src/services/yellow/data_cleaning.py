"""Yellow taxi data cleaning implementation."""
...
def _compute_duration_seconds(df: pl.DataFrame) -> pl.Series:
    ...
    pickup = df["tpep_pickup_datetime"].cast(pl.Datetime)
    dropoff = df["tpep_dropoff_datetime"].cast(pl.Datetime)
    return (dropoff - pickup).dt.total_seconds()


def _quality_rules(df: pl.DataFrame) -> dict[str, int]:
    """Check data quality rules and return violation counts."""
...
class YellowDataCleaning(BaseDataCleaning):
    """Data cleaning for yellow taxi data."""

    def analyze(self, df: pl.DataFrame) -> StepResult:
        """Run data cleaning on yellow taxi dataframe.
...
if __name__ == "__main__":
    ...
    result = YellowDataCleaning().analyze(df=sample)
