"""FHVHV taxi fare revenue analysis implementation."""
...
_DROPOFF_COL = "dropoff_datetime"
_DRIVER_PAY_COL = "driver_pay"
_SURCHARGE_COLS = [
    "tolls",
    "bcf",
    "sales_tax",
...
def _daily_revenue(df: pl.DataFrame) -> pl.DataFrame:
    """Aggregate daily total revenue from base_passenger_fare."""
    return (
        ...
        .group_by("day")
        .agg(pl.col(_FARE_COL).sum().alias("daily_revenue"))
        .sort("day")
    )
...
def _fare_anomalies(df: pl.DataFrame) -> dict[str, int]:
    ...
    for col in [_FARE_COL, _DRIVER_PAY_COL]:
...
def _tip_prediction(df: pl.DataFrame) -> dict:
    """Regression to predict tips from trip_miles, duration, base_passenger_fare."""
...
class FhvhvFareRevenueAnalysis(BaseFareRevenueAnalysis):
    """Fare revenue analysis for FHVHV taxi data."""

    def analyze(self, df: pl.DataFrame) -> StepResult:
        """Run fare revenue analysis on FHVHV taxi dataframe.
...
if __name__ == "__main__":
    ...
    sample = pl.DataFrame(
        {
            ...
            _DRIVER_PAY_COL: rng.uniform(low=3.0, high=50.0, size=n).tolist(),
    ...
    result = FhvhvFareRevenueAnalysis().analyze(df=sample)
