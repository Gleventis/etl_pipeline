"""FHVHV taxi geospatial analysis implementation."""
...
def _distance_by_zone(df: pl.DataFrame) -> list[dict]:
    """Distance distribution statistics per pickup zone using trip_miles."""
    if _DIST_COL not in df.columns:
        return []

    stats = (
        df.filter(pl.col(_DIST_COL).is_not_null())
        .group_by(_PU_COL)
        .agg(
            pl.col(_DIST_COL).mean().alias("mean_distance"),
            pl.col(_DIST_COL).median().alias("median_distance"),
            pl.col(_DIST_COL).std().alias("std_distance"),
    ...
    return [
        {
            ...
            if row["mean_distance"] is not None
            else 0.0,
            "median_distance": float(row["median_distance"])
            if row["median_distance"] is not None
            else 0.0,
            "std_distance": float(row["std_distance"])
            if row["std_distance"] is not None
            else 0.0,
            "trip_count": int(row["trip_count"]),
        }
...
class FhvhvGeospatialAnalysis(BaseGeospatialAnalysis):
    """Geospatial analysis for FHVHV taxi data.

    Uses trip_miles for distance distribution instead of trip_distance.
    """

    def analyze(self, df: pl.DataFrame) -> StepResult:
        """Run geospatial analysis on FHVHV taxi dataframe.
        ...
        distance_dist = _distance_by_zone(df=df)
        ...
        detail = {
            ...
            "distance_by_zone": json.dumps(distance_dist),
    ...
    sample = pl.DataFrame(
        {
            ...
            _DIST_COL: rng.uniform(low=0.5, high=30.0, size=n).tolist(),
        }
    )
    result = FhvhvGeospatialAnalysis().analyze(df=sample)
