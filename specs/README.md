...
**Branching pipeline variant**: DAG-based pipeline topology replacing the linear sequential chain. Steps declare explicit `after` dependencies, enabling parallel branches (geospatial + temporal run concurrently after cleaning, fare revenue joins both). DSL grammar/validation changes, Prefect code generation for parallel tasks, DAG-aware scheduler resume logic. [Spec](branching_pipeline_variant.md) | [Plan](branching_pipeline_variant_implementation_plan.md)
