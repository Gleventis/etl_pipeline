"""Yellow taxi geospatial analysis implementation."""
...
def _distance_by_zone(df: pl.DataFrame) -> list[dict]:
    """Distance distribution statistics per pickup zone."""
...
class YellowGeospatialAnalysis(BaseGeospatialAnalysis):
    """Geospatial analysis for yellow taxi data."""

    def analyze(self, df: pl.DataFrame) -> StepResult:
        """Run geospatial analysis on yellow taxi dataframe.
    ...
    sample = pl.DataFrame(
        {
            ...
            _DIST_COL: rng.uniform(low=0.5, high=20.0, size=n).tolist(),
        }
    )
    result = YellowGeospatialAnalysis().analyze(df=sample)
