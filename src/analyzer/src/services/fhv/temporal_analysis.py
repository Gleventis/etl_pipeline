"""FHV taxi temporal analysis implementation."""
...
def _build_hourly_series(df: pl.DataFrame) -> pl.DataFrame:
    """Aggregate trip counts by hour (no fare columns for FHV)."""
    return (
...
class FhvTemporalAnalysis(BaseTemporalAnalysis):
    """Temporal analysis for FHV taxi data.

    No fare-based temporal aggregations since FHV has no fare columns.
    """

    def analyze(self, df: pl.DataFrame) -> StepResult:
        """Run temporal analysis on FHV taxi dataframe.
...
if __name__ == "__main__":
    ...
    result = FhvTemporalAnalysis().analyze(df=sample)
